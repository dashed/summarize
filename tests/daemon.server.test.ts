...
import { resolveGitSha, resolvePackageVersion } from "../src/version.js";

describe("daemon/server health payload", () => {
  it("includes daemon version, commit, and pid", () => {
    ...
    expect(payload.commit).toBe(resolveGitSha(import.meta.url) ?? null);
  });
});
